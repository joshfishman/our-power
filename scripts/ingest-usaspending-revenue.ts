...
async function run(): Promise<void> {
  ...
  // A federal fiscal year N runs 1 Oct (N-1) to 30 Sep (N). If we are pulling the
  // year we are currently inside, its total is a part-year figure and must never
  // be rendered next to closed years without saying so — otherwise eleven months
  // of FY2026 next to twelve of FY2025 reads as a decline that did not happen.
  const retrieved = new Date().toISOString().slice(0, 10);
  const partialFiscalYear = retrieved <= `${flags.endFy}-09-30` ? `FY${flags.endFy}` : null;
  ...
  const payload = {
    ...
    /** Set when the last fiscal year pulled has not closed yet; null when every year is complete. */
    partial_fiscal_year: partialFiscalYear,
    entities: results,
  };
