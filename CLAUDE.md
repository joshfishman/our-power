...
- **Integrate on `dev`, never push to `main`.** All work lands on the `dev` branch (push feature branches into `dev`, or push to `dev` directly). Do NOT push to `main` — `main` is production and the user ships it themselves. The path to production is `dev` → `main`, performed by the user ("I will deploy"). Keep an open `dev` → `main` PR so the user can merge when they choose; never merge it yourself without an explicit instruction.
