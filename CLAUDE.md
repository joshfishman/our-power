...
- **Committing and pushing is fine without asking.** `git commit` and `git push` (to a branch) do not count as deploying — go ahead. Deploying to Vercel still requires an explicit instruction per the rule above.
