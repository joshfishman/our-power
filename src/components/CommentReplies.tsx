...
export function CommentReplies({ parentId }: { parentId: number }) {
  ...
  if (isPending) return <p className="text-sm font-semibold text-muted-foreground">Loading replies...</p>;
  if (isError) return <p className="text-sm font-semibold text-muted-foreground">Error loading replies.</p>;
