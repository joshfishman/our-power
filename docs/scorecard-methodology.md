...
| v1.7.7  | 2026-05-29 | **Receipts-denominator fix.** Audit (#31, #32) caught that `ingest-fec-receipts-multicycle` silently dropped any (legislator, cycle) pair the FEC bulk `/candidates/totals/` endpoint didn't return. Senators on 6-year terms (Markey, Booker, Sanders) were getting 1 cycle of 4; first-time candidates (McCormick) 0 of 4 — collapsing their denominator and producing nonsense scores (McCormick = 6, driven entirely by $14.86M IE_SUPPORT). Two changes: (1) the ingest now falls back to per-candidate `/candidate/{id}/totals/` for any legislator below `MIN_EXPECTED_ROWS=1` after the bulk pass — captures every cycle the PCC actually filed for. (2) `getPacScoresByLegislatorV171` and `getLegislatorMoneyTrail` add a safety guard: `totalReceipts = 0 ∧ countsAgainst > 0 → pacScore = null` ("no data") rather than computing a score from IE alone. Belt-and-braces. |
