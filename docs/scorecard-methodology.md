...
| v1.8.2  | 2026-05-29 | **Individual vs PAC % fix: scoped to 2022–2024 cycles to match individual data window.** The "Individual vs PAC" tile on the legislator detail page was dividing 2-cycle `totalItemized` (2022 + 2024 only — we never ingested 2018/2020 individual files) by `(totalItemized + 4-cycle PAC totalInfluence)`, structurally biasing every legislator toward "PAC-heavy." New helper `getLegislatorPacInfluence_2022_2024` sums `PacContribution` for `cycleYear IN (2022, 2024)` and `kind IN ('DIRECT','IE_SUPPORT')`. PAC Score (4-cycle) is unchanged. Spot-check: Scalise 37% → 56%, Sanders 97% → 99%, McCormick 43% → 43% (no 2018/2020 PAC dollars to drop).                                                                                                              |
