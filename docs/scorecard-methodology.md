...
| v1.8.3  | 2026-05-29 | **Industry classifier fix + coverage transparency.** (1) Employer-industry classifier: HEALTH rule's `HOSPITAL` stem was matching "HOSPITALITY" — pulling 39 hotel/REIT/restaurant-group employers (Hersha Hospitality Trust, Vesta Hospitality, Union Square Hospitality Group, etc.) into Healthcare instead of Retail/Hospitality. Fixed via negative lookahead `HOSPITAL(?!ITY)` so the HOSPITAL stem stops short of "HOSPITALITY" and lets the later RETAIL_HOSPITALITY rule catch them. (2) `IndividualMoneySection` UI: industry mix is computed from the stored top-25 employers per cycle, which for big-fundraising senators (McCormick: $27M itemized → $2M classified, 6.1% coverage) is a single-digit % of the full base. Header and footnote now surface coverage explicitly ("X% of $Y itemized") so per-industry % isn't read as a share of the whole donor base. |
